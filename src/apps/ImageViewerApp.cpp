...
void ImageViewerApp::drawBrowser() {
    ...
    UIFramework::clearArea(framebuffer, 0, 0, w, LIB_TOP_H);
    
    typography.setFontSize(40.0f);
    typography.renderText("Image Viewer", 70, 2, framebuffer);
    
    // Draw Settings Cog in top-right
    UIFramework::drawIcon16x16(framebuffer, 920, 18, COG_ICON, 0x00);
    DisplayHAL::drawHLine(0, LIB_TOP_H - 1, w, 0x00, framebuffer);
    ...
    UIFramework::clearArea(framebuffer, 0, LIB_MAIN_Y, LIB_PAGING_W, LIB_MAIN_H);
    DisplayHAL::drawRect(0, LIB_MAIN_Y, LIB_PAGING_W, LIB_MAIN_H, 0x00, framebuffer);
    ...
    typography.renderText(upSymbol, 10 + (pBtnW - twUp) / 2, upY + (pBtnH - symbolHeight) / 2, framebuffer);
    typography.renderText(dnSymbol, 10 + (pBtnW - twDn) / 2, dnY + (pBtnH - symbolHeight) / 2, framebuffer);
    ...
    typography.renderText("Exit to", LIB_SIDE_X + (LIB_SIDE_W - extW1) / 2, LIB_MAIN_Y + 20, framebuffer);
    typography.renderText("Launcher", LIB_SIDE_X + (LIB_SIDE_W - extW2) / 2, LIB_MAIN_Y + 50, framebuffer);

    // Render JPG file list
    UIFramework::clearArea(framebuffer, LIB_LIST_X, LIB_MAIN_Y, LIB_LIST_W, LIB_MAIN_H);
    ...
    for (int i = 0; i < itemsPerPage; i++) {
        ...
        if (idx < (int)m_files.size()) {
            ...
            typography.renderText(name, LIB_LIST_X + 20, rowY + 25, framebuffer);
    ...
    if (m_files.empty()) {
        typography.setFontSize(26.0f);
        typography.renderText("No JPG images found in /images", LIB_LIST_X + 20, LIB_MAIN_Y + 50, framebuffer, 0x04);
    }

    DisplayHAL::display(framebuffer);
}

void ImageViewerApp::drawConverting() {
    int w = DisplayHAL::getWidth();
    int h = DisplayHAL::getHeight();
    ...
    typography.setFontSize(40.0f);
    typography.renderText("Converting Image...", 50, 100, framebuffer);

    typography.setFontSize(24.0f);
    typography.renderText("Path: " + m_selectedPath, 50, 180, framebuffer, 0x03);
    typography.renderText("Status: " + m_statusMsg, 50, 220, framebuffer, 0x04);
    ...
    if (progressW > 0) {
        DisplayHAL::fillRect(barX + 2, barY + 2, progressW, barH - 4, 0x00, framebuffer);
    }

    // Progress Text
    std::string pct = std::to_string(m_progress) + "%";
    int pctW = typography.measureText(pct);
    typography.renderText(pct, barX + (barW - pctW) / 2, barY + barH + 10, framebuffer);
...
void ImageViewerApp::drawView() {
    ...
    // 2. Load converted raw 4-bit E-Ink format directly into framebuffer
    ...
    // 3. Draw a Back navigation overlay button (bottom right)
    int backX = w - 180;
    int backY = h - 60;
    int backW = 160;
    int backH = 48;
    ...
    typography.renderText("Exit View", backX + (backW - tw) / 2, backY + 12, framebuffer);
...
void ImageViewerApp::update() {
    ...
    if (m_conversionStep == 1) {
        ...
        if (!m_rgbData) {
            m_statusMsg = "Error: JPG decoding failed.";
            m_state = STATE_BROWSER;
            drawBrowser();
            return;
        }
        ...
        // Convert 18 lines per update cycle (takes 30 updates to complete 540 lines)
        int linesToConvert = 18;
        int endLine = m_conversionLine + linesToConvert;
        if (endLine > 540) endLine = 540;

        for (int y = m_conversionLine; y < endLine; y++) {
            int src_y = y * m_imgHeight / 540;
            for (int x = 0; x < 960; x++) {
                int src_x = x * m_imgWidth / 960;
...
#else
        // Ensure parent directory exists (VFS root /data)
        if (!SD.exists("/data")) {
            SD.mkdir("/data");
        }
        if (SD.exists(m_cachePath.c_str())) {
            SD.remove(m_cachePath.c_str());
        }
        File out = SD.open(m_cachePath.c_str(), FILE_WRITE);
        if (out) {
